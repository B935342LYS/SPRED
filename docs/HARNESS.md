...
- Practice effect bonus first pass is connected for gliss only: active-track `GlissEvent` targets are split into fixed `250ms` intervals, each interval can add a small `Gliss!` bonus when the current frame is voiced, failures have no penalty, result/live UI keeps separated `Gliss!` / `Vib!` / `Trem!` counts plus `effectBonusScore`; vib/trem detection remains follow-up
...
- the deployment staging repo `regression-code-test-publish/` has been synced through commit `f7d21bf Update practice results and tuplet gliss rendering`; the latest active result-grid grouping should be synced separately if it is required in the deployed build
...
- continue modifier bonus judging for vib/trem after the gliss-only effect bonus prototype is checked with real microphone input
