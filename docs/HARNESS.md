...
- next Examples work is to update the Edge Function and frontend so empty access word loads `access_group = "public"` examples, valid access word loads `public + tester`, and wrong non-empty access word preserves the current score and current list while showing an error; then continue verification with all Supabase Storage score files, public `score_url` direct-open checks, manifest list rendering, each Example Load path, broken JSON/URL failure preservation, and practice-mode lock behavior
