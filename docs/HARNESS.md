...
- Game pitch visualization first pass is implemented: `pitchy` dependency, microphone pitch frame loop, clarity/RMS/frequency range filtering, runtime `GamePitchFrame`, green pitch dot overlay near the playback boundary, active target pitch-class octave-only correction, `+100ms` target lookahead for overlay candidate lookup, and `npm run test:game` pitch math coverage
- Practice judging stabilization is connected: pitch dot x position now aligns with the playback boundary, scoring uses the same hysteresis correction path as the pitch overlay, short detector pitch-class glitches such as `E4 -> C6/C#6` are held at the previous stable target for a `500ms` grace window, and `test:game` covers octave-equivalent scoring plus glitch hold/release behavior
- Practice judge feedback overlay is connected: each scoring sample displays `Perfect`/`Ok`/`Bad`/`Miss` text above the hit target note for `500ms`, `Perfect`/`Ok` include `COMBO n`, `Bad`/`Miss` hide combo text, and text uses outline shadow without a background rectangle
- Practice accuracy display now uses one decimal place in the live panel and result dialog
...
- the deployment staging repo `regression-code-test-publish/` has been synced through commit `d528b25 Expand copyright notice`
...
- latest verified commands: `npm run test:game`, `npm run test:audio`, `npm run typecheck`, `npm run build`, and `npx tsc --noEmit --noUnusedLocals --noUnusedParameters`; deployment staging verification also included `npm run typecheck` and `npm run build`; previous broader verification also included `npm run test:score`, `npm run test:parse`, `npm run test:edit`, `npm run test:view`, `npm run test:analyze`, `npm run test:layout`, and `npm run test:youtube`
