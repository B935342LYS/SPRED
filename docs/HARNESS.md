...
- Practice accuracy display uses one decimal place in the live panel and result dialog, and `Miss` samples now count as `0%` in the average accuracy denominator
- Practice timing judgment is connected: onset candidates come from `unvoiced -> voiced` or sufficient pitch-class change, `80-249ms` offsets show small `early` / `late`, `250ms+` timing miss downgrades the final pitch label to `Miss`, each note/onset is consumed once, and very short same-pitch consecutive notes avoid forced timing downgrade
...
- the deployment staging repo `regression-code-test-publish/` has been synced through commit `3082a6f Tune practice judge overlay size`
