...
- latest verified commands: `npm run test:game`, `npm run test:track`, `npm run typecheck`, `npm run build`, and `git diff --check`; previous broader verification also included `npm run test:score`, `npm run test:parse`, `npm run test:edit`, `npm run test:view`, `npm run test:analyze`, `npm run test:audio`, `npm run test:layout`, `npm run test:youtube`, and `npx tsc --noEmit --noUnusedLocals --noUnusedParameters`
