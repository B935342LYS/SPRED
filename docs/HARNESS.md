...
- current focus has moved from track/audio/YouTube stabilization to first user-test deployment follow-up, practice-mode stabilization, and 2.11 Examples / Manual support planning
...
- `docs/2.11-example-score-and-manual-support-spec.md` now defines the first Examples implementation as Supabase Edge Function based rather than local static example based: no local example Score JSON fallback, temporary access-word gate, reduced catalog table, current-score preservation on remote failure, and a dedicated `src/app/examples/` module boundary
...
- Help > Manual now opens a first-pass static manual at `regression-code/public/manual/index.html`
...
- the deployment staging repo `regression-code-test-publish/` has been synced and pushed through commit `74e4ec4 Add full combo result display`; this includes the gliss-neighbor target selection fix and FULL COMBO result display
...
- latest verified commands: `npm run test:examples`, `npm run typecheck`, and `npm run build`; previous broader verification also included `npm run test:game`, `npm run test:score`, `npm run test:parse`, `npm run test:edit`, `npm run test:view`, `npm run test:analyze`, `npm run test:layout`, `npm run test:audio`, `npm run test:youtube`, and `npx tsc --noEmit --noUnusedLocals --noUnusedParameters`
...
- continue Examples verification with all 16 Supabase Storage score files: generated catalog SQL review, public `score_url` direct-open checks, manifest list rendering, each Example Load path, wrong access word behavior, broken JSON/URL failure preservation, and practice-mode lock behavior
