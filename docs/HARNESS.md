...
- Practice judging is connected: each scoring sample displays `Perfect`/`Ok`/`Bad`/`Miss` text above the hit target note for `500ms`, `Perfect`/`Ok` include `COMBO n`, `Bad`/`Miss` hide combo text, text uses outline shadow without a background rectangle, combo text is always white, and the current overlay font sizes are combo `15px`, label `21px`, timing `12px`
...
- Practice effect bonus first pass is connected for gliss only: active-track `GlissEvent` targets are split into fixed `250ms` intervals, each interval can add a small `Gliss!` bonus when the current frame is voiced, failures have no penalty, result/live UI keeps separated `Gliss!` / `Vib!` / `Trem!` counts plus `effectBonusScore`; `Gliss!` overlay now appears separately to the right of the main judge/combo text, uses the target gliss pitch for note-near placement even with octave-shifted input, and is styled as smaller yellow text; vib/trem detection remains follow-up
...
- latest verified commands: `npm run test:game`, `npm run typecheck`, `npm run build`, and `npx tsc --noEmit --noUnusedLocals --noUnusedParameters`; deployment staging verification also included `npm run test:game`, `npm run typecheck`, and `npm run build`; previous broader verification also included `npm run test:score`, `npm run test:parse`, `npm run test:edit`, `npm run test:view`, `npm run test:analyze`, `npm run test:layout`, `npm run test:audio`, and `npm run test:youtube`
...
- continue modifier bonus judging for vib/trem after the gliss-only effect bonus prototype is checked with real microphone input; first vib candidate is target-near average pitch plus roughly `±25 cent` oscillation, minimum duration and direction-change checks, while trem should reuse RMS envelope/onset data without adding heavy real-time analysis
